...
export default function CentrosCusto() {
  ...
  const renderTreeItem = (item: CentroCustoNode, level: number = 0) => {
    ...
    return (
      <div key={item.id} className="select-none">
        <div 
          className={`flex items-center justify-between p-3 hover:bg-[var(--color-bg-main)] border-b border-[var(--color-border)] ${level > 0 ? 'ml-6 border-l pl-4' : ''}`}
        >
          <div className="flex items-center gap-2 flex-1">
            {hasChildren ? (
              <button 
                onClick={() => toggleExpand(item.id)}
                className="p-1 hover:bg-[var(--color-border)] rounded text-[var(--color-text-muted)]"
              >
            ...
            <span className="font-mono text-sm text-[var(--color-text-muted)] bg-[var(--color-bg-main)] px-1 rounded mr-2">
              {item.codigo || '------'}
            </span>
            <span className="font-medium text-[var(--color-text-main)]">{item.nome}</span>
            {item.descricao && (
              <span className="text-sm text-[var(--color-text-muted)] ml-2 hidden md:inline-block">- {item.descricao}</span>
            )}
          </div>
          <div className="flex items-center gap-2 text-sm text-[var(--color-text-muted)]">
            ...
            <button 
              ...
              className="p-1 hover:bg-[var(--color-success)]/10 text-[var(--color-success)] rounded transition-colors"
            ...
            <button 
              ...
              className="p-1 hover:bg-[var(--color-primary)]/10 text-[var(--color-primary)] rounded transition-colors"
              title="Editar"
            >
              <Edit2 size={16} />
            </button>
            <button
              ...
              className="p-2 text-[var(--color-danger)] hover:bg-[var(--color-danger)]/10 rounded-full transition-colors"
              title="Excluir"
            >
              <Trash2 size={18} />
            </button>
  ...
  if (isLoading) return <div className="p-8 text-center text-[var(--color-text-muted)]">Carregando...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[var(--color-bg-paper)] rounded-xl shadow-sm border border-[var(--color-border)] overflow-hidden">
        <div className="p-4 border-b border-[var(--color-border)] bg-[var(--color-bg-main)] font-medium text-[var(--color-text-secondary)]">
